...
class Fighter():
    ...
    def __init__(self,x,y,flip:bool,data,sprite_sheet,animation_steps,sound,running:pygame.mixer.Sound,different=False,height=None) -> None:
        self.size = data[0]
        self.scale = data[1]
        self.offset = data[2]
        ...
        if different:
            self.animation_list = self.load_images_diff(sprite_sheet,animation_steps)
    ...
    def load_images_diff(self,images,animation_steps):
        try:
            animation_list = []
            for i,x in enumerate(images):
                temp_img_list = []
            
                for step in range(animation_steps[i]):
                    if self.height:

                        temp_img = x.subsurface(step*self.size,0,self.size,self.height)
                        temp_img_list.append(pygame.transform.scale(temp_img,(self.size * self.scale, self.size* self.scale)))
                    else:
                        temp_img = x.subsurface(step*self.size,0,self.size,self.size)
                        temp_img_list.append(pygame.transform.scale(temp_img,(self.size * self.scale, self.size* self.scale)))
        ...
        if self.attacking is False:
            if self.player_2:
                ...
                if key[pygame.K_RIGHT]:
                    self.running = True
                    dx = SPEED
                if key[pygame.K_RSHIFT] or key[pygame.K_END]:
